	...
	$.ajax({
		...
		.done(function(args){
			$("#position option").each(function() {	
			 ...
			 $("#position").append("<option value=''>::직급선택::</option>");
			 
			 for(var idx=0; idx<args.length; idx++) {	
				 $("#position").append("<option value='"+args[idx].position+"'>"+args[idx].position+"</option>");	
			 } 
		})
	    .fail(function(e) {
	...
	$.ajax({
		...
		.done(function(args){
			$("#name option").each(function() {	
			 ...
			 for(var idx=0; idx<args.length; idx++) {	
				 $("#name").append("<option value='"+args[idx].name+"'>"+args[idx].name+"</option>");	
			 } 
		})
	    .fail(function(e) {
...
function selectNumber(){
	...
	var params="dept_no="+dept_no+"&position="+position+"&name="+name;
	...
	$.ajax({
		...
		.done(function(args){
			$("#employee_no option").each(function() {	
			 ...
			 $("#employee_no").append("<option value='0'>::사번선택::</option>");
			 
			 for(var idx=0; idx<args.length; idx++) {	
				 $("#employee_no").append("<option value='"+args[idx].employee_no+"'>"+args[idx].employee_no+"</option>");	
			 } 
		})
	    .fail(function(e) {
...
function pushList(){
	...
	var params="dept_no="+dept_no+"&position="+position+"&name="+name+"&employee_no="+employee_no;
		...
		.done(function(args){
			...
			$("#empList2").empty();
			 ...
			 for(var idx=0; idx<args.length; idx++) {	
				 ...
				 $("#empList2").append("<tr><td id =부서"+idx+" align='center'>"+args[idx].dept_name+"</td><td id=직위"+idx+" align='center'>"+args[idx].position+"</td><td id=이름"+idx+" align='center'>"+args[idx].name
						 +"</td><td align='center'><select id=authority"+idx+" name=authority><option value='"+args[idx].authority+"'>"+args[idx].authority+"</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option></select></td></tr>");
			 } 
        ...
        <div id="nav">
        	...
        	<div id="sidemenu">
        		<ul>
        			...
        			<li><a href="list.do">부서관리</a></li><br>
...
<center>
<p><h3>조 회</h3></p>
<table width="780" frame="void"><tr><td width="200">
<label for="dept_name"><b>부서명&nbsp:&nbsp</b></label>
<select id="dept_name" name ="dept_name" onchange="selectDept()">	
 <option value="">::부서선택::</option></select></td>


<td width="180"><label for="position"><b>직급&nbsp:&nbsp</b> </label>
<select id="position" name="position" onchange="selectName()">
  <option value="">::직급선택::</option></select></td>

<td width="200"><label for="name"><b>사원선택&nbsp:&nbsp </b></label>
<select id="name" name="name" onchange="selectNumber()">
  <option value="">::사원선택::</option></select></td>

<td width="200"><label for="employee_no"><b>사번선택&nbsp:&nbsp </b></label>
<select id="employee_no" name="employee_no">
  <option value="0">::사번선택::</option></select></td>
</tr>
</table>
<br>
...
<thead><tr><th width="200" align="center">부 서</th><th width="200" align="center">직 위</th><th width="200" align="center">이 름</th><th width="100" align="center">권 한</th></tr></thead>
<tbody id ='empList2'>
...
<button type ="button" style="width:70px;" id="test" name="test" onclick="test();">수 정</button>
